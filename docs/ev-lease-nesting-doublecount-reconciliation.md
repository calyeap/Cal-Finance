...
`CF-LEASE-NESTING-DOUBLECOUNT-RECON-01` (issue #311). This document does
what the OUTCOME asks: determine whether current approved authority already
determines a correction to `computeEnterpriseValue`
(`lib/analyzer/modules/enterpriseValue.ts:52`) for the first defect
`docs/ev-double-count-sizing.md` reports and does not fix — the §3.5 bridge
adding `finance-lease-liabilities` on top of a `total-debt` figure that, for
a nested filer, already contains it — and, if not, return exactly one closed
`CALVIN REQUIRED` question.

**This document reconciles and reports. It rules nothing, and it changes no
product code.** All work was at `c01f57c` (the head this outcome started
from, `#310` merged).

**The determination: no correction is authority-determined. This ends on one
closed `CALVIN REQUIRED`, per SCOPE item 2/CALVIN REQUIRED's own gate.**
...
`calboard-secmap-2026-09-3`, unchanged). Full output:
`.evidence/ev-double-count/sizing.txt`.
...
**Nothing moves, because nothing was changed.** This outcome ends on SCOPE
2's determination with zero product-code diff (`git diff` against `c01f57c`
touches only this document and `docs/ev-double-count-sizing.md`'s pointer
prose, per SCOPE 7). `gates.leverage`, `fairValueRange`, `trust.status` and
verdict status for MSFT, OKLO and NVDA are therefore identical to what they
were before this outcome ran, by construction rather than by re-verification.
...
**Question:** which route, if any, should be authorised to build the
nesting-determination mechanism `docs/lease-once-measurement.md` §6 leaves
open, so a future outcome can correct the §3.5 bridge's proven $105M/UNP
double-count?

**Why this is not a restatement of the 2026-09-09 ruling.** That ruling
settled *what `total-debt` means* for a nested filer. This is the
downstream question it explicitly left open: *how a filer's nested status
is established in software*, without which the settled meaning cannot be
applied to any specific company's bridge. Nothing above reopens the
2026-09-09 ruling, the rejection of the two superseded inferences, the
unconditional INCOMPLETE verdict rule (issue #205), or `CALVIN RULING — A`
on period coherence.

**The smallest closed set of options**, per `filing-text-evidence-routes.md`
§6, each restated with its consequence:

1. **Route 1 — fetch the filing's XBRL instance document and calculation
   linkbase**, scoped to the two figures this defect needs
   (`total-debt`, lease-inclusive combined elements). Resolves 2 of 10
   deterministically and machine-checkably (UNP via element identity, RIVN
   via disjoint sibling summands); MSFT, OKLO and COST get prose delivered
   but not determined without a further read. No §4.4 amendment, no AI
   permission question. Cost: a new document type, an XBRL instance parser
   and a calculation-linkbase parser — `filing-text-evidence-routes.md` §2
   calls this a milestone, not a bounded fix.
2. **Route 2 — a fourth §4.4-style recorded judgment.** Resolves 6 of 10
   (MSFT, OKLO, COST, UNP, RIVN, LLY — LLY resolving to "nested, amount
   unknown," i.e. still INCOMPLETE but for the right reason). Requires
   amending §4.4's frozen enumeration of exactly three judgments — a
   `docs/frozen/` change, which is independently a HARD BOUND of *this*
   outcome and would need to be lifted explicitly for a future one. Cost:
   no new dependency, but a standing per-company-per-period human-review
   obligation.
3. **Route 3 — the §3.8.1 documented AI-extraction fallback.** Resolves the
   same 6 of 10 as Route 2, by the same reasoning (both read the issuer's
   note prose). Requires first settling whether a nesting classification
   qualifies as "that figure" under §3.8.1's fallback permission — unsettled
   on the text today (§1 above; `filing-text-evidence-routes.md` §4). Cost:
   opens the acquisition path's `[C]` layer to a REQUIRED EV input for the
   first time, with the silent-and-systematic failure mode §3 of that
   document exists to guard against.
4. **Authorise none of the above now; leave the defect reported and
   uncorrected.** No route is built. UNP's bridge (and any future
   UNP-shaped nested filer) keeps adding a lease already inside its debt
   figure whenever its other REQUIRED inputs are eventually supplied.
   Today this is exclusively latent: no calibration company's EV completes
   regardless of this defect, because the §4.4 non-operating-investments
   judgment is unrecorded for the whole set
   (`ev-double-count-sizing.md`'s own "What 'today' means"). The $105M /
   0.35% UNP exposure stays exactly where this and the prior pass placed
   it, cited but not corrected, until a later outcome is separately
   authorised to lift one of the three HARD BOUNDS above.

**Ending.** Per this outcome's TERMINAL CONTRACT, this reconciliation ends
on the closed `CALVIN REQUIRED` above. No line of `lib/`, `scripts/`, or any
byte-unchanged artefact changed. The only diffs are this document and the
pointer added to `docs/ev-double-count-sizing.md`'s first-defect prose
(SCOPE 7), which restates nothing about the sizing itself.
