...
1. **Confidence computation — RESOLVED (Option B), 21 Sep 2026.** Calvin's ruling ([#196 comment](https://github.com/calyeap/Cal-Finance/issues/196#issuecomment-5759425335), 21 Sep 2026 11:01:20Z), verbatim: *"Use the existing deterministic `TrustStatus` as a qualitative evidence-status label instead of inventing a numeric confidence score. Keep the §10.6.4 action clause separate/deferred until its M8 dependency exists. Do not merge the action clause into the top-level BUY / HOLD / SELL verdict block and do not invent new confidence math, thresholds, scoring, or trade policy. Proceed within existing authority. Blocker C is decided."* No numeric verdict-level confidence figure is or may be computed — that question stays exactly as unresolved as before this ruling; nothing here re-derives, adjusts or extends §9.6's `computeTrustStatus`. `ConfidenceIndicator` (§3) instead renders the already-computed §9.6 `TrustStatus` (`CLEAN` / `PARTIAL` / `UNUSABLE`) through a pure copy mapping, on the same completed-verdict-only condition this contract already settled, with copy bounds against reading it as a statistical certainty claim or as a judgement on the company rather than the analysis. Implemented by M9-CONFIDENCE-LABEL-01 (issue #201).
2. **The §10.6.4 entry-side action clause vs. the top-level verdict.** `m9-contract-reconciliation.md` §6: *"no source reconciles the two into one UI element, and this outcome does not invent that reconciliation."* This contract places the action clause inside `ScenarioRangeStrip` (Overview slot 4, §2.1) as part of the valuation-evidence layer, and the verdict inside `DominantVerdictSlot` (slot 2) — adjacent on the same page, per #118's fixed ordering, but this contract does not assert any further UI relationship (a shared component, a cross-reference, a combined rendering) between them beyond that shared-page adjacency, which follows directly from #118's own fixed slot order rather than from a new design decision.
   ...
   **DISPOSITION — ANSWERED, `CF-M9-BLOCKER14-RECON-01`, 24 Sep 2026.** Calvin's ruling ([#196 comment](https://github.com/calyeap/Cal-Finance/issues/196#issuecomment-5759425335), 21 Sep 2026 11:01:20Z) — already quoted verbatim in item 1 above — settles exactly this question: *"Keep the §10.6.4 action clause separate/deferred until its M8 dependency exists. Do not merge the action clause into the top-level BUY / HOLD / SELL verdict block."* The Analyzer V2 cutover (`CF-DESIGN-AUTHORITY-CUTOVER-01`, `docs/design/analyzer-v2-design-authority.md`) does not reopen it: `DominantVerdictSlot` and `ScenarioRangeStrip` remain two separate components (`app/components/AnalyzerReportFrame.tsx:105,112`), both rendered inside the shared `az-hero` block — the same shared-page adjacency this item already described, carried into the V2 shell rather than superseded by it. No shared component, cross-reference or combined rendering exists in the tree today. This item is closed on the ruling; the V2 cutover changed the surrounding layout, not this relationship.
3. **Market Context's content source — RESOLVED, 21 Sep 2026.** Calvin's ruling ([#195](https://github.com/calyeap/Cal-Finance/issues/195#issuecomment-5759160247), 21 Sep 2026 10:39:55Z, Option B) approved the already-acquired SEC SIC / `sicDescription` classification (`AcquiredCompany.sic`/`sicDescription`) as Market Context's scoped source: rendered as explicit category framing, never a peer, index or sector-average comparison, and no new vendor, peer-comps service, market-data provider or broader Market Context engine is authorised by it. The same ruling approved the filer's own 10-K Item 1 narrative, under a fixed extraction rule, as the Business section's source. Implemented by M9-ITEM5-CONTENT-01. What remains open beyond that narrow bound — any genuinely comparative Market Context — is a separate, unasked decision this ruling does not make.
4. **M9 acceptance validation of the `INCOMPLETE`-by-default state.** §2.1 records that, under the current M8 dependency, `DominantVerdictSlot` will render `INCOMPLETE` for every run until M8 lands. This contract instructs that the state be designed and validated as first-class (§2.1), but whether that validation happens before or is blocked on M8 remains a sequencing question outside this contract's HARD BOUNDS.
   ...
   **DISPOSITION — NARROWED BY EVIDENCE, `CF-M9-BLOCKER14-RECON-01`, 24 Sep 2026.** The `INCOMPLETE`-by-default state has in fact been designed and validated as first-class, on real acquired data, and that validation now reaches the current V2 surface, not only the historical M9 routes: `app/components/analyzerSurfacesOnRealRun.test.tsx` renders real MSFT and OKLO runs through `AnalyzerReportFrame` itself and asserts the shared hero shows the `INCOMPLETE` state name, `verdict.reason` verbatim as the cause line, and no confidence figure; `lib/analyzer/automaticAnalysisOnRealRun.test.ts` confirms both real companies reach that same `INCOMPLETE` result through the fully automatic pipeline, with no human write; `lib/analyzer/m9RealCompanyValidationGuards.test.ts` pins `verdict.ts` to its `INCOMPLETE`-only content by hash and guards that the unified route (`app/analyzer/[runId]/page.tsx`) never redirects a run into a human step instead of rendering that state. What this evidence cannot and does not validate — because it cannot yet occur — is the `BUY` / `HOLD` / `SELL` branches once M8's comparator lands; that part of the sequencing question stays genuinely blocked on M8, not open or actionable now. So item 4 is satisfied for the state this contract could actually ask for validation of today; it is not, and cannot yet be, satisfied for the branches M8 alone can produce.
