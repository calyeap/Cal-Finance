...
**Implemented shape.** `lib/analyzer/verdict.ts:39` declares `export type VerdictStatus = "BUY" | "HOLD" | "SELL" | "INCOMPLETE"`. `deriveVerdict` (verdict.ts:52-81) returns `{ status: "INCOMPLETE", reason: <string> }` on every path currently reachable (confirmed by `lib/analyzer/verdict.test.ts`, which asserts `INCOMPLETE` for all six exercised cases, including the "the comparator has not been acquired yet" case that fires unconditionally once trust and range are usable). Only one of the four `INCOMPLETE` branches states a recovery path in its `reason` string; the other three state a cause only:

| Branch | `reason` source | Recovery path stated? |
|---|---|---|
| `COMPARATOR_NOT_YET_AVAILABLE` (verdict.ts:46-50, returned at 77-80) | fixed string ending *"Recovery: this verdict becomes available once M8 delivers the comparator fact."* | yes |
| `trust.status === "UNUSABLE"` (verdict.ts:53-59) | `` `Decision-critical analysis is incomplete — ${detail}` ``, where `detail` comes from `lib/analyzer/trust.ts` (e.g. `"PROFILE NOT CONFIRMED on the valuation path"`, line 119; `` `INCOMPLETE on ${appliesTo}` ``, line 146) | no |
| `range.kind === "suppressed"` (verdict.ts:63-68) | `` `Decision-critical analysis is incomplete — ${range.cause}` `` | no |
| `range.kind === "pre-revenue-distribution"` (verdict.ts:70-75) | fixed string, *"The pre-revenue valuation model does not yet define a BUY / HOLD / SELL boundary."* | no |

Nothing downstream adds a recovery path either: both render sites (below) print `verdict.reason` verbatim and nothing else.

**Does the shape satisfy the contract, or diverge? Split answer.** On the not-a-fourth-verdict half, it satisfies #118 and the Decision Log on every path: `INCOMPLETE` is never treated as a peer investment conclusion, and no code path manufactures BUY/HOLD/SELL or a HOLD default in its absence (this is exactly what PR #140 / CF-V2-PROOF-01 was built to enforce, and `verdict.test.ts` pins it). On the "carries... a recovery path" half, it **diverges**: per the table above, three of the four reachable branches state a cause only, with no recovery path anywhere in the returned `reason` or in its renderers. This is exactly the question SCOPE item 3 commissioned, so the divergence is recorded rather than smoothed over; closing it (adding a recovery clause to the other three branches, or deciding recovery belongs elsewhere) is not decided here — see §6. Structurally, `INCOMPLETE` is one member of the same `VerdictStatus` union as the three real verdicts — a type-level grouping, not a rendering decision. Today's only consumer, `app/analyzer/[runId]/report/page.tsx:81-82` (and its snapshot-page equivalent, `app/analyzer/[runId]/snapshot/[version]/page.tsx:45-46`), renders `Verdict — {verdict.status}` and `{verdict.reason}` through the same two CSS classes (`.name` / `.cause`) regardless of which status it is — there is currently no distinct visual treatment separating a completed verdict from the failure state. That is an open point for the M9 design contract to rule on, not something this outcome resolves: the Decision Log's own UI requirement elsewhere (on a related state, suppression/qualification) is that such states *"require distinct visual treatment,"* which is a reasonable analogy but is not itself a ruling on `INCOMPLETE`'s rendering. This reconciliation records the gap; it does not close it.
...
- A decision-critical failure renders as an explicit `INCOMPLETE` state carrying a stated cause, never a manufactured verdict and never a default HOLD (§3).
...
- That the `INCOMPLETE` state carries a recovery path — three of its four reachable branches currently state a cause only, with no recovery path anywhere in `verdict.reason` or its renderers (§3); only the not-a-fourth-verdict / no-manufactured-HOLD guarantee holds on every path.
